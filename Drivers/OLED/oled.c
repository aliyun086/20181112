...
{
	uint8_t cmd_buf[]={0x00,dat};
	uint8_t dat_buf[]={0x40,dat};
	if(cmd)
	{
		HAL_I2C_Master_Transmit(&hi2c1, OLED, cmd_buf, 2, 10);  //OK,µ«ÊÇÆÁÄ»Ë¢ÐÂÂÊºÜµÍ
		//HAL_I2C_Master_Transmit_DMA(&hi2c1, OLED, cmd_buf, 2);
	}else{
		HAL_I2C_Master_Transmit(&hi2c1, OLED, dat_buf, 2, 10);
		//HAL_I2C_Master_Transmit_DMA(&hi2c1, OLED, dat_buf, 2);
	}
}
